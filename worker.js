...
async function applyPass(jobId,env,mode) {
  ...
  for (const page of pages) {
    ...
    try {
      ...
      html=mode==="animations"?injectAnimationPass(removeAnimationPass(html)):injectImageSliderPass(removeImageSliderPass(html));
      await env.STORAGE.put(key,html,{httpMetadata:{contentType:"text/html; charset=utf-8"},customMetadata:{jobId,pageId:page.id,sourceUrl:page.source_url,version:VERSION,lastStaticPass:mode}});
  ...
  await env.DB.prepare("UPDATE migration_jobs SET current_stage=?,progress_percent=?,updated_at=? WHERE id=?").bind(success?stage:`${stage}_with_warnings`,success?99:98,new Date().toISOString(),jobId).run();
...
function removeAnimationPass(html){return html.replace(/<style\b[^>]*id=["']static-migrator-animation-final-v16[67]["'][^>]*>[\s\S]*?<\/style>/gi,"").replace(/<script\b[^>]*id=["']static-migrator-animation-final-v16[67]-script["'][^>]*>[\s\S]*?<\/script>/gi,"")}
function injectAnimationPass(html){const payload=`
...
<script id="static-migrator-dm-image-slider-v168-script">(function(){function placeholder(el){return (el.textContent||'').replace(/\\s+/g,' ').trim().toLowerCase()==='slide title'}function run(){document.querySelectorAll('.dmImageSlider[data-widget-type="imageSlider"],.dmImageSlider').forEach(function(slider){var slides=Array.from(slider.querySelectorAll('ul.slides>li'));if(!slides.length)return;var active=slides.find(function(li){return li.classList.contains('flex-active-slide')})||slides.find(function(li){return li.style.display!=='none'&&li.getAttribute('aria-hidden')!=='true'})||slides[0];slider.setAttribute('data-static-migrator-frozen','true');slides.forEach(function(li){li.removeAttribute('data-static-migrator-active-slide')});active.setAttribute('data-static-migrator-active-slide','true');active.classList.add('flex-active-slide');active.style.setProperty('display','block','important');active.style.setProperty('opacity','1','important');active.style.setProperty('visibility','visible','important');active.style.setProperty('transform','none','important');active.style.setProperty('z-index','2','important');var cover=active.querySelector('.dmCoverImgContainer');var image=active.querySelector('img[src]');if(cover&&image){var src=image.currentSrc||image.getAttribute('src');if(src)cover.style.setProperty('background-image','url("'+src.replace(/"/g,'%22')+'")','important');cover.style.setProperty('display','block','important')}active.querySelectorAll('.slide-title').forEach(function(title){if(placeholder(title))title.style.setProperty('display','none','important')});var inner=active.querySelector('.slide-inner');if(inner){inner.style.setProperty('display','block','important');inner.style.setProperty('opacity','1','important');inner.style.setProperty('visibility','visible','important');inner.style.setProperty('transform','none','important');inner.style.setProperty('animation','none','important')}})}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',run,{once:true});else run();window.addEventListener('load',run,{once:true})})();</script>`;return appendToBody(html,payload)}
function appendToBody(html,payload){return /<\/body>/i.test(html)?html.replace(/<\/body>/i,`${payload}\n</body>`):`${html}\n${payload}`}

async function servePreview(jobId,requestedPath,env){const job=await env.DB.prepare("SELECT * FROM migration_jobs WHERE id=?").bind(jobId).first();if(!job)return new Response("Preview job not found.",{status:404});let path;try{path=decodeURIComponent(requestedPath||"/").replace(/^\/+/,"")}catch{return new Response("Invalid preview path.",{status:400})}if(!path)path="index.html";if(path.endsWith("/"))path+="index.html";if(path.includes("..")||path.includes("\\"))return new Response("Invalid preview path.",{status:400});const choices=[path];if(!/\.[a-z0-9]{1,10}$/i.test(path))choices.push(`${path}/index.html`);let object=null,resolved=null;for(const choice of choices){object=await env.STORAGE.get(`${job.output_prefix}/site/${choice}`);if(object?.body){resolved=choice;break}}if(!object?.body||!resolved)return new Response("Preview file not found.",{status:404});const type=object.httpMetadata?.contentType||contentType(resolved);const headers=new Headers({"Content-Type":type,"Cache-Control":"no-store","X-Robots-Tag":"noindex,nofollow"});const prefix=`/preview/${encodeURIComponent(jobId)}`;if(/text\/html/i.test(type))return new Response(prefixUrls(await object.text(),prefix),{status:200,headers});if(/text\/css/i.test(type))return new Response(prefixCss(await object.text(),prefix),{status:200,headers});return new Response(object.body,{status:200,headers})}
